...
def main(args):
    ...
    llm_model = Qwen2_5_VLForConditionalGeneration.from_pretrained(args.pretrained_model_name_or_path, torch_dtype=weight_dtype, cache_dir=args.cache_dir)
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        with torch.no_grad():
            # encode text prompt
            messages = create_messages(images)
            output_prompts = [processor.apply_chat_template(msg, tokenize=False, add_generation_prompt=True) for msg in messages]
            ...
            generated_ids = llm_model.generate(**inputs, max_new_tokens=128)
