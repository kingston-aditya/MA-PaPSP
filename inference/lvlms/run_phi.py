...
def main(args):
    ...
    pipe, train_dataloader = accelerator.prepare(pipe, train_dataloader)
    ...
    for _, batch in enumerate(tqdm(train_dataloader, desc="Inferring")):
        ...
        with torch.no_grad(), torch.amp.autocast():
            ...
            output = pipe(output_prompts, **generation_args)
            neg_txt_caps.append(output[0]['generated_text'])

    np.save(os.path.join(args.output_dir, f"{args.dataset_name}_phi3b_negtxt_caps.npy"), np.array(neg_txt_caps))
